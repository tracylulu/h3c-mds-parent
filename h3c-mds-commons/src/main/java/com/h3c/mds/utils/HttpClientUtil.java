...
public class HttpClientUtil {
	...
	public String getUserImageBase64(String loginUrl,String httpUrl,String account,String password) {
		JSONObject obj = new JSONObject(sendHttpPostJson(loginUrl, "{\"account\":\""+account+"\",\"password\":\""+password+"\"} "));
		int code = Integer.parseInt(obj.toMap().get("code").toString()) ;
		if(code != 200)
		{
		...
		Map<String,String> dataMap = (Map<String, String>) obj.toMap().get("data");
		String authorization = dataMap.get("token") ;
		HttpGet httpGet = new HttpGet(httpUrl);
		httpGet.addHeader("Authorization",  "Bearer "+authorization);
		JSONObject result = new JSONObject(sendHttpGet(httpGet));
		//System.out.println(sendHttpGet(httpGet));
		String base64 = (String) result.toMap().get("data");
	...
	public static String getData(String loginUrl,String account,String password,String method,String httpUrl) throws Exception {
		...
		int code = Integer.parseInt(obj.toMap().get("code").toString()) ;
		if(code != 20220) {
			throw new Exception(obj.toMap().get("message").toString());
		}
		Map<String,String> dataMap = (Map<String, String>) obj.toMap().get("data");
		String authorization = dataMap.get("token") ;
	...
	public static void main(String[] args) throws Exception {
		// 登录
		String loginUrl = "http://api.eos-ts.h3c.com/user/v1.0/account/token";
		String httpUrl = "http://api.eos.h3c.com/orgbaseinfo/v1.0/employee/image?empCode=12328";
		httpUrl="http://api.eos-ts.h3c.com/orgbaseinfo/v1.0/views/ibds_depts";

		System.out.println(getDataPost(loginUrl,"ibds","Hello123",httpUrl));
//		JSONObject obj = new JSONObject(sendHttpPostJson(loginUrl, "{\"account\":\"ibds\",\"password\":\"Hello123\"} "));
//		int code = Integer.parseInt(obj.toMap().get("code").toString()) ;
//		if(code != 200)
//		{
//			System.out.println("login ok");
