...
public class FinanceQuartzJobBean extends QuartzJobBean{

	
	
	private Logger LOG = LoggerFactory.getLogger(FinanceQuartzJobBean.class);
...
//	@Override
	protected void executeInternal(JobExecutionContext context)
		...
		LOG.info("产品编码开始向研发基本信息和财务参考信息中同步数据---定时任务");
		...
		if(CollectionUtils.isNotEmpty(ibdsTasks)){
			for(int i = 0; i < ibdsTasks.size(); i++){
				...
				//产品编码
				if(FlowableConst.TASK_TYPE_PRODUCTCODE_FINANCE_AND_PRODUCT.equalsIgnoreCase(ibdsTasks.get(i).getType())){
					if(StringUtils.isBlank(ibdsTasks.get(i).getStatus())){
						LOG.info("产品编码开始向研发基本信息和财务参考信息中同步数据");
