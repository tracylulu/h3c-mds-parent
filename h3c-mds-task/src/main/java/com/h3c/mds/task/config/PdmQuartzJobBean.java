...
public class PdmQuartzJobBean extends QuartzJobBean {
...
//		同步和华智的数据到新增part编码
		handleHzDataToPart(FlowableConst.GETDATAFROM_FROM_HZ);
...
//        logger.info("....execute:{}....", context.getTrigger().getKey());
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
	private void handleHzDataToPart(String type){
		Strategy pdmStrategy = StrategyFactory.createStrategyImpl(type);
		List<HzPartView> pdmList = pdmStrategy.getSourceData(null);
		List<PartProcessAndPartCodeWrapper> wrapperList = pdmStrategy.transferPOJO(pdmList);
		pdmStrategy.saveData(wrapperList);
	}
}
