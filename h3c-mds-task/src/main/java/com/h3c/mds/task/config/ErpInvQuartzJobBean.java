...
public class ErpInvQuartzJobBean extends QuartzJobBean{

	
	
	private Logger LOG = LoggerFactory.getLogger(ErpInvQuartzJobBean.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		if(CollectionUtils.isNotEmpty(ibdsTasks)){
			for(int i = 0; i < ibdsTasks.size(); i++){
				if(FlowableConst.TASK_TYPE_PRODUCTCODE_INV.equalsIgnoreCase(ibdsTasks.get(i).getType()) 
						|| FlowableConst.TASK_TYPE_PART_INV.equalsIgnoreCase(ibdsTasks.get(i).getType())){
