...
<body>
	...
	<p>
	<p>
		<span class="title191030">四级部门信息 </span>
	</p>
	...
	<script type="text/javascript">
		...
		function initForm${id_suffix}(){
			$.post('/dept/getYIJiDeptListByCodeAndName?code=${code}',function (result) {
			 ...
			 $.ajax({
		            url:'/dept/judgeAddSiJiDept?code=${code}&level=${level}',
		            type:"get",
		            dataType:"text",
		            success:function(data){
		            	//alert(data);
		                 if(data=='ok'){
		                	 ...
		                	 if(data=='error'){
		                		 //alert("当前用户只能维护SysPersonRole表中设置的一级部门及以下部门信息！");
				...
				success : function(data) {				
					...
					if(obj.success){
						if(isQuit){
							...
							$('#btnQuit${id_suffix}').linkbutton('enable');
							var tab = $('#mainUrl').tabs('getSelected');		
						    tab.panel('refresh', "/dept/saveTo4List/${code}?tid=${tid}");
						}						
