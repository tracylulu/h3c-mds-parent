...
<script type="text/javascript">
        ...
        $('#baseInfoFileForm${timestamp}').form('submit', {
            ...
            success: function(str){
                if(JSON.parse(str).status == 200){
                    ...
                    $("#basisMaterial_"+index+"_${timestamp}").val(data.fileName);
                    $("#basisMaterialId_"+index+"_${timestamp}").val(data.fileId);
                }else{
    ...
    function deleteFile${timestamp}(index){
        ...
        $.post("/flowable/file/deleteFile",{"fileId":fileId},function (result) {
            if(result.status==200){
                ...
                $("#basisMaterial_"+index+"_${timestamp}").val("");
                $("#basisMaterialId_"+index+"_${timestamp}").val("");
            }else{
    ...
    function downloadFile${timestamp}(index) {
        ...
        var fileId = $("#basisMaterialId_"+index+"_${timestamp}").val()
        var url = "/flowable/file/downloadFile?fileId="+fileId+"&fileName="+encodeURIComponent(fileName);
        debugger;
