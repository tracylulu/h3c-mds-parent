...
<body>
	...
	<p style="text-align: center;">
			<span style="font-size: 20px;">${title}日志 </span>
		</p>
		<br>
		...
		$(function(){
			...
			$("#log${id_suffix}").datagrid({
				url: "product/log/listdata?code=${code}",		        
		        ...
		        columns: [[
		        	...
		        	{field: "code",title: "所属内码",align: "center",width:'20%',formatter:formatProgress${id_suffix}},
		        	{field: "name",title: "所属版本",align: "center",width:'20%',formatter:formatProgress${id_suffix}},
		...
		function getEditBefore${id_suffix}(id){
			$("#log1${id_suffix}").propertygrid({
				url : 'product/log/getEditBefore?code=${code}&id='+id,/*<!--***url需要更改 ***--> */
			showGroup : true,
			scrollbarSize : 0,
			title:"变更前数据",
	...
	        ]]
		...
		function getEditAfter${id_suffix}(id){
			$("#log2${id_suffix}").propertygrid({
				url : 'product/log/getEditAfter?code=${code}&id='+id,/*<!--***url需要更改 ***--> */
			showGroup : true,
			scrollbarSize : 0,
			title:"变更后数据",
			height : window.innerHeight
