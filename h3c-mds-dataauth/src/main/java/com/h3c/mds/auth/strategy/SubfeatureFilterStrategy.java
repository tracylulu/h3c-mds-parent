...
@Scope("prototype")
@Component("subfeatureFilterStrategy")
public class SubfeatureFilterStrategy extends AbstractFilterStrategy{
    @Override
    protected TreeLevel getTreeLevel() {
        if(this.filterParam instanceof MapperMethod.ParamMap){
            ...
            if(param.get("id") == null){
                ...
                return new TreeLevel(FilterConstant.LEVEL_TYPE_TREE,"IBDS_FEATRUE","FEATURENo","Readers","prodlineno");
            }else if(param.get("id") != null && "FT".equals(param.get("type"))){
                return new TreeLevel(FilterConstant.LEVEL_TYPE_BASE,"IBDS_SUB_FEATRUE","SUBFEATURENo","Readers","subfeatureno");
            }
        }
        return null;
    }
