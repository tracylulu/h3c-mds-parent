...
@Scope("prototype")
@Component("moduleVersionFilterStrategy")
public class ModuleVersionFilterStrategy extends AbstractFilterStrategy{
    @Override
    protected TreeLevel getTreeLevel() {
        if(this.filterParam instanceof MapperMethod.ParamMap){
            ...
            if(param.get("id") == null){
                ...
                return new TreeLevel(FilterConstant.LEVEL_TYPE_TREE,"IBDS_MODULE","MODULENo","Readers","prodlineno");
            }else if(param.get("id") != null && "MD".equals(param.get("type"))){
                return new TreeLevel(FilterConstant.LEVEL_TYPE_BASE,"IBDS_MODULE_VERSION","MDLVSNNo","Readers","mdlvsnno");
            }
        }
        return null;
    }
