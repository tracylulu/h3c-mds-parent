...
import com.h3c.mds.flowable.dao.PartPdmNotSyncMapper;
...
@Service("pdmNotSyncStrategy")
public class PdmNotSyncStrategyImpl extends DefaultAbstractStrategyImpl{

	
	@Autowired
    private PartPdmNotSyncMapper partPdmNotSyncMapper;
	
	@Autowired
	private UserService userService;
	@SuppressWarnings("unchecked")
	@Override
	public List<PartPdmNotSync> doGetSourceData(Object f) {
		List<String> pdmList = (List<String>)f;
		PartPdmNotSyncExample example = new PartPdmNotSyncExample();
		PartPdmNotSyncExample.Criteria criteria = example.createCriteria();
		criteria.andDocumentnameIn(pdmList);
		return partPdmNotSyncMapper.selectByExample(example);
	}
	...
	public List transferPOJO(List fromPOJOs) {	
		...
		if(fromPOJOs != null && fromPOJOs.size() > 0){
			for(int i = 0; i < fromPOJOs.size(); i++){
				...
				PartPdmNotSync pdmPartView = (PartPdmNotSync)fromPOJOs.get(i);
	...
	private PartProcessAndPartCodeWrapper buildPartProcessAndPartCodeWrapper(PartPdmNotSync pdmPartView){
