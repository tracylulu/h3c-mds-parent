...
public  class AbstractExcelReader implements ExcelReader{
     ...
     */
    @Override
    public  void checkExcelValid(File file) throws FlowableException {
        // System.out.println("filename:" + file.getName());
        if (!file.exists()) {
            // 文件不存在
            throw new FlowableException("文件不存在");
        } else if (!(file.isFile() && (file.getName().endsWith(EXCEL_XLS) || file.getName().endsWith(EXCEL_XLSX)))) {
            throw new FlowableException("不是标准的Excel文件");
        }
    }
     ...
     */
    @Override
    public  void initWorkBook(InputStream inputStream, String fileName) throws FlowableException,IOException {
